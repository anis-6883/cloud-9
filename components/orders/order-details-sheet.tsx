                    ...
                    <div className='flex flex-col items-center'>
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all ${
                          step.completed ? `${stepConfig.bgColor} ${stepConfig.textColor}` : "bg-secondary text-muted-foreground"
                        }`}
                      >
                        {step.completed ? "✓" : index + 1}
                      </div>
                      {!isLast && <div className={`w-1 h-8 mt-2 transition-all ${step.completed ? "bg-accent" : "bg-border"}`} />}
                    </div>
                    <div className='pt-1 flex-1'>
                      <p className={`font-medium ${step.completed ? "text-foreground" : "text-muted-foreground"}`}>{stepConfig.label}</p>
                      {step.timestamp && (
          ...
          <div className='bg-gradient-to-br from-accent/20 to-accent/10 rounded-xl p-4 border border-accent/30'>
            <div className='flex gap-4 items-center'>
              <div className='relative w-12 h-12 rounded-full overflow-hidden flex-shrink-0'>
                <Image src={order.deliveryPerson.avatar} alt={order.deliveryPerson.name} fill className='object-cover' />
              </div>
              <div className='flex-1 min-w-0'>
