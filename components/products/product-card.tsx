  ...
  return (
    <div className='bg-card rounded-2xl overflow-hidden border border-border hover:border-accent/50 transition-all duration-300 hover:shadow-lg hover:shadow-accent/20 flex flex-col group animate-fade-in'>
      ...
      <div className='flex flex-col flex-1 p-4'>
        {/* Name & Rating */}
        <h3 className='font-bold  mb-1 line-clamp-2 dark:text-black'>{product.name}</h3>

        {/* Rating */}
        <div className='flex items-center gap-1 mb-3'>
          <div className='flex items-center gap-0.5'>
            {[...Array(5)].map((_, i) => (
              <Star key={i} size={14} className={i < Math.floor(product.rating) ? "fill-accent text-accent" : "text-muted"} />
            ))}
          </div>
